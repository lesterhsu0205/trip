...
- **重要1： 每次的回應必須嚴格遵守主 [CLAUDE.md](./CLAUDE.md) 和 [子專案](#2025hokkaido---北海道行程) 內 CLAUDE.md**
- **重要2：你是一個旅遊專家，必須實事求是**：
...
- [ ] **1. 交通資訊正確性**
  - 針對當日全部行程內的下列 URL 執行 WebFetch 取得最新資料比對驗證：
    - [子專案](#2025hokkaido---北海道行程)/[Google Maps URL](./2025/hokkaido/CLAUDE.md#google-maps-url)
...
- [ ] **2. 遞迴檢查當日行程時序**
  - 依序檢查下列 4 個子項目：
    ...
    <!-- - [ ] **2.4 大眾交通銜接時間**：大眾交通銜接時間必須充足（僅限大眾交通，至少 10 分鐘 buffer） -->

- [ ] **3. 行程前購票及預定等整理**
  - 使用 Grep 搜尋 todo.md 相關項目（購票、預訂），避免完整讀取文件
- [ ] **4. 行程中購物資訊及地點整理**
  - 根據[購物地點要求](#購物地點要求)，使用 Grep 搜尋 remark.md 相關項目，避免完整讀取文件
<!-- - [ ] **5. 行程中大眾交通資訊整理**
...
**適用情境**：WebFetch/WebSearch 工具失敗或取得資訊有誤時
    ...
    - **查詢標準流程**：
      1. **平行查詢所有路線（必須）**：
         - **強制要求**： 一次發送所有交通段的查詢，不可輪詢
           ...
           - 效益：總時間 = 最慢的一次查詢（而非累加）
         - 優先：查詢 [子專案](#2025hokkaido---北海道行程)/[官方參考交通資料](./2025/hokkaido/CLAUDE.md#官方參考交通資料)
      ...
      3. 依照 [子專案](#2025hokkaido---北海道行程)/[交通資訊查詢結果格式](./2025/hokkaido/CLAUDE.md#交通資訊查詢結果格式)標註
  - **票價資訊**： 門票、交通費
  - **特殊限制**： 年齡限制、推車限制、預約需求
- ⚠️ **建議使用**：餐廳評價、景點最新資訊
- ❌ **不需使用**：
...
├── cspell.json                  # 拼寫檢查配置（包含日文地名等專有名詞）
├── .github/workflows/           # GitHub Actions 自動化腳本
│   └── merge-itinerary.yml      # 自動合併行程檔案的 CI/CD 流程
└── 2025/
    ├── hokkaido/                # 北海道專案（11/14-11/19）
    │   ├── README.md            # 自動合併的完整行程
    │   ├── CLAUDE.md            # 子專案專屬指引
    │   ├── todo.md              # 行程前購票及預定等整理
...
- 每個旅遊專案都有獨立的目錄（按年份和目的地組織）
- 每個專案目錄內包含專屬的 `CLAUDE.md` 檔案，提供該專案的具體指引
- 主要的 `CLAUDE.md` 檔案提供通用指導原則
- 各日行程使用 `D1.md`、`D2.md` 等格式命名
- `README.md` 為自動合併的完整行程檔案，**不應手動編輯**
- 支援多專案平行開發，每個專案有獨立的旅遊時間和需求
...
- **觸發條件**：推送 `D*.md`、`todo.md`、`remark.md`、`README.md`、`CLAUDE.md`、`transportation.md` 檔案時
- **功能**：自動偵測變更的專案目錄，合併各日行程檔案到 `README.md`
- **處理流程**：
  1. 偵測變更的專案目錄
  2. 合併所有 `D*.md` 檔案並調整標題層級（H1 → H2）
  3. 附加 `todo.md`、`remark.md`、`transportation.md`（如存在）
  4. 添加最後更新時間戳記
  5. 自動提交變更並推送

### 拼寫檢查配置

- `cspell.json` 包含日文地名、車站名、店名等專有名詞
- 配置支援旅遊相關的專有名詞拼寫檢查
...
專為 2025 年 11/14-11/19 的北海道之旅設計，詳細資訊請參考 [2025/hokkaido/CLAUDE.md](/2025/hokkaido/CLAUDE.md)

### 2025/tokyo - 東京輕井澤行程

專為 2025 年 10/6-10/13 的東京輕井澤之旅設計，詳細資訊請參考 [2025/tokyo/CLAUDE.md](/2025/tokyo/CLAUDE.md)
